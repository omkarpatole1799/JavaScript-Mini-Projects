...
const account1 = {
    owner: "Omkar Patole",
    movements: [66, 222, -99, 1000, -6, -30, 225, 1633], // movements: [10, -5, 10],
    interestRate: 0.9,
    password: 1,
    movementsDates: ["Sun Mar 05 2023 13:09:20 GMT+0530 (India Standard Time)", "Sun Mar 03 2023 13:09:20 GMT+0530 (India Standard Time)", "Sun Feb 13 2023 13:09:20 GMT+0530 (India Standard Time)", "Sun Feb 12 2023 13:09:20 GMT+0530 (India Standard Time)", "Sun Feb 11 2023 13:09:20 GMT+0530 (India Standard Time)", "Sun Feb 10 2023 13:09:20 GMT+0530 (India Standard Time)", "Sun Feb 09 2023 13:09:20 GMT+0530 (India Standard Time)", "Sun Feb 08 2023 13:09:20 GMT+0530 (India Standard Time)",],
    locale: "pt-PT", // de-DE
    currency: "USD",
};
...
const displayMovements = function (account, sorting = false) {
    const transactions = sorting ? account.movements.slice().sort((a, b) => a - b) : account.movements;
    ...
    transactions.forEach(function (movement, i) {
        ...
        const date = formatedDate(new Date(account.movementsDates[i]), currentUser.locale);
        ...
        <div class="movements__value">${formatLocaleMoney(currentUser.locale, currentUser.currency, movement.toFixed(2))}</div>
    </div>`;
...
const displayBalance = function (account) {
    ...
    labelBalance.textContent = formatLocaleMoney(currentUser.locale, currentUser.currency, account.balance.toFixed(2));
};
...
const displaySummary = function (account) {
    ...
    labelSumIn.textContent = formatLocaleMoney(currentUser.locale, currentUser.currency, depositSummary.toFixed(2));
    ...
    labelSumOut.textContent = formatLocaleMoney(currentUser.locale, currentUser.currency, withdrawalSummary.toFixed(2));
    ...
    labelSumInterest.textContent = formatLocaleMoney(currentUser.locale, currentUser.currency, interestSummary.toFixed(2));
};
...
btnTransfer.addEventListener("click", function (e) {
    ...
    if (reciverAccount && currentUser.balance >= sendingAmount && sendingAmount > 0 && reciverAccount.userName !== currentUser.userName) {
...
btnClose.addEventListener("click", function (e) {
    e.preventDefault();
    if (inputCloseUsername.value === currentUser.userName && Number(inputClosePin.value) === currentUser.password) {
        const index = accounts.findIndex(() => inputCloseUsername.value === currentUser.userName);
...
btnLoan.addEventListener("click", function (e) {
    ...
    if (loanAmount > 0 && currentUser.movements.some((movement) => {
